    ...
    for (category in incl_infra_type) { # each infra type
        ...
        for (i in 1:nrow(data)) { # each row
            ...
            for (year_x in start_year:end_year) { # each list element
                ...
                # C1: inst year present and year_x occurs before inst yearr
                condition_1 <-
...
```{r func4}
calc_len_by_yearxinfra <- function(
        df,
        year_start_var = 2009,
        year_end_var = 2021,
...
```{r func5}
plot_len_by_yearxinfra <- function(
        df,
        title = "",
        xlabel = "",
        ylabel = "",
    ...
    aes(
        ...
        order = dplyr::desc(Infrastructure_Type)
    )
) +
        ...
        geom_segment( # 50km red triangle point down
            aes(
                x = 2009,
                y = 50.01 - (max(Total_Length) * 0.05),
                xend = 2009,
                yend = 50 - (max(Total_Length) * 0.05),
                color = "#bb0000",
                hjust = 0.15
            ),
            arrow = arrow(
        ...
        geom_segment( # 50km red triangle point up
            aes(
                x = 2009,
                y = (max(Total_Length) * 0.05) - 0.01,
                xend = 2009,
                yend = (max(Total_Length) * 0.05),
                color = "#bb0000",
                hjust = 0.15
            ),
            arrow = arrow(
...
```{r vanc-data}
vanc <- read_csv("../data/vancouver-bikeways-2009-2022.csv")
vanc %>%
    select(ID_CITY,
           INST_YR,
           INST_TYPE,
...
```{r calg-data}
calg <- read_csv("../data/calgary-bikeways-2009-2022.csv")
calg %>%
    select(ID_CITY,
           INST_YR,
           INST_TYPE,
...
```{r toron-data}
toron <- read_csv("../data/toronto-bikeways-2009-2022.csv")
toron %>%
    select(ID_CITY,
           INST_YR,
           INST_TYPE,
...
```{r len-by-yearxinfra-plot, fig.width=8.5, fig.height=11}

# Combine all cities for plotting
vanc_plot_infra <- calc_len_by_yearxinfra(vanc, 2009, 2021) %>%
    plot_len_by_yearxinfra(title = "Vancouver, Canada") +
    theme(plot.margin = unit(c(15,5,5,5), "point"))
calg_plot_infra <- calc_len_by_yearxinfra(calg, 2009, 2022) %>%
    plot_len_by_yearxinfra(title = "Calgary, Canada")
toron_plot_infra <- calc_len_by_yearxinfra(toron, 2009, 2022) %>%
    plot_len_by_yearxinfra(title = "Toronto, Canada")
...
grid.arrange(
    ...
    ncol = 1,
    top = textGrob(
        "Roadways with Dedicated Cycling Infrastrcture",
        gp = gpar(fontsize=18)
    ),
    bottom = textGrob(
        "Years (2009-2022)",
