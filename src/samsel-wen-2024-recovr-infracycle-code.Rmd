...
toron <- toron_bikeways %>%
	...
	rename(ATR_SEGMENT_TYPE = FEATURE36)
toron
```
...
```{r}
arterial_group <- c("Arterial")
collector_group <- c("Collector", "Secondary Arterial")
local_group <- c("Lane", "Residential", "Leased", "Recreational")
```

## Toronto, CA

```{r}
arterial_group <- c("Major Arterial", "Major Arterial Ramp", "Minor Arterial")
collector_group <- c("Collector")
local_group <- c("Local", "Other")
```

## Calgary, CA

```{r}
arterial_group <- c("Arterial Street", "Industrial Arterial", "Local Arterial", "Parkway", "Urban Boulevard")
collector_group <- c("Neighbourhood Boulevard", "Collector", "Primary Collector", "Skeletal Road")
local_group <- c("Access Route", "Residential Street", "Activity Center Street", "Historic Road Allowance", "Lanes (Alleys)", "Industrial Street")
```
