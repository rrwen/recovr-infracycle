...
vanc_preprocess <- vanc_raw %>%
	...
	mutate( # create col for canbics orig installs
		install_type_pl_ct = case_when(
			install_type == "Protected Bike Lanes" & road_type != "Off-street" ~ "CT",
			install_type == "Painted Lanes" & road_type != "Off-street" ~ "PL",
			install_type %in% c("Shared Lanes", "Local Street") & road_type != "Off-street" ~ "SR",
			.default = NA
		)
	) %>%
