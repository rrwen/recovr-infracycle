...
tab1$data_raw <- vanc %>%
	...
	select(city, install_type, install_type_pl_ct, verify_install_type, road_type) %>%
	add_row(
		calg %>%
			mutate(city = "calgary") %>%
			select(city, install_type, install_type_pl_ct, road_type)
	) %>%
	add_row(
		toron %>%
			mutate(city = "toronto") %>%
	...
	filter(!is.na(install_type_pl_ct))

# Calc municipal lengths
tab1$data <- tab1$data_raw %>%
	group_by(city, install_type_pl_ct) %>%
	summarize(
...
tab1$data <- tab1$data %>% left_join(
	tab1$data_raw %>%
		...
		as_tibble %>%
		select(-geometry),
	by = join_by(city, install_type_pl_ct == verify_install_type)
)
...
tab1$data <- tab1$data %>% left_join(
	tab1$data_raw %>%
		as_tibble %>%
		select(-geometry) %>%
		mutate( # create 1 or 0 if municipal matches verify
			install_type_match = install_type_pl_ct == verify_install_type
		) %>%
		group_by(city, install_type_pl_ct) %>%
		summarize(
			len_km_municipal = (sum(st_length(geometry), na.rm = T) / 1000) %>% as.numeric,
...
tab1$data <- tab1$data  %>%
	mutate(
		install_type_pl_ct = case_when(
			install_type_pl_ct == "PL" ~ "Painted Lane",
			install_type_pl_ct == "CT" ~ "Cycle Track",
			install_type_pl_ct == "SR" ~ "Shared Road",
			install_type_pl_ct == "LSB" ~ "Local Street Bikeway",
			.default = NA
		)
	) %>%
	rename(
		City = city,
		Classification = install_type_pl_ct,
		Municipal = len_km_municipal,
		Verified = len_km_verify,
		Difference = len_km_diff
	)
...
tab1$data <- tab1$data %>%
	...
	rename(Measure = install_type_pl_ct) %>%
	mutate(
