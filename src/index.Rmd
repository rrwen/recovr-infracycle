...
#' @param len_per Units of road length (numeric) to normalize by (e.g. 1000 means per 1000 km of roadway). Set to `NULL` to disable normalization of road lengths.
#' @return Multiple area ggplots of the cumulative yearly road lengths by infrastructure type combined with patchwork.
#' @export
#'
plot_yearly_len_infra <- function(df_list, len_per = NULL) {
	...
	y_title <- ggplot() +
		annotate(
			...
			x = 1,
			y = 1,
			label = if (!is.null(len_per)) glue("Length {len_per}") else "Total Length (Centreline km)",
			angle = 90,
			size = 5
		) +
...
plot_yearly_len_infra(list(
	"Vancouver, CA" = vanc,
	"Vancouver, CA (without Local Street Bikeways)" = vanc %>% 
		filter(verify_install_type != "LSB"),
	"Calgary, CA" = calg,
	"Toronto, CA" = toron
))
```
