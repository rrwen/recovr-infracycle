...
* [vancouver-bikeways-raw-v2.geojson](https://github.com/rrwen/recovr-infracycle/blob/main/data/vancouver-bikeways-raw-v2.geojson)
* [vancouver-bikeways-raw-v2.csv](https://github.com/rrwen/recovr-infracycle/blob/main/data/vancouver-bikeways-raw-v2.csv)
...
* [calgary-bikeways-raw-v3.geojson](https://github.com/rrwen/recovr-infracycle/blob/main/data/calgary-bikeways-raw-v3.geojson)
* [calgary-bikeways-raw-v3.csv](https://github.com/rrwen/recovr-infracycle/blob/main/data/calgary-bikeways-raw-v3.csv)

**Note**: Only segments with verified installations are shown (n = `r calg_raw %>% filter(!is.na(INST_TMIN)) %>% nrow` of `r calg_raw %>% nrow`).
...
* [toronto-bikeways-raw-v3.geojson](https://github.com/rrwen/recovr-infracycle/blob/main/data/toronto-bikeways-raw-v3.geojson)
* [toronto-bikeways-raw-v3.csv](https://github.com/rrwen/recovr-infracycle/blob/main/data/toronto-bikeways-raw-v3.csv)
...
* [vancouver-bikeways-preprocess-v3.geojson](https://github.com/rrwen/recovr-infracycle/blob/main/data/vancouver-bikeways-preprocess-v3.geojson)
* [vancouver-bikeways-preprocess-v3.csv](https://github.com/rrwen/recovr-infracycle/blob/main/data/vancouver-bikeways-preprocess-v3.csv)
...
# Save geojson
vanc_preprocess %>%
	write_sf("../data/vancouver-bikeways-preprocess-v3.geojson", delete_dsn = TRUE)
...
vanc_preprocess %>%
	...
	write_csv("../data/vancouver-bikeways-preprocess-v3.csv", na = "")
...
* [calgary-bikeways-preprocess-v4.geojson](https://github.com/rrwen/recovr-infracycle/blob/main/data/calgary-bikeways-preprocess-v4.geojson)
* [calgary-bikeways-preprocess-v4.csv](https://github.com/rrwen/recovr-infracycle/blob/main/data/calgary-bikeways-preprocess-v4.csv)
...
# Save geojson
calg_preprocess %>%
	write_sf("../data/calgary-bikeways-preprocess-v4.geojson", delete_dsn = TRUE)
...
calg_preprocess %>%
	...
	write_csv("../data/calgary-bikeways-preprocess-v4.csv", na = "")
...
* [toronto-bikeways-preprocess-v4.geojson](https://github.com/rrwen/recovr-infracycle/blob/main/data/toronto-bikeways-preprocess-v4.geojson)
* [toronto-bikeways-preprocess-v4.csv](https://github.com/rrwen/recovr-infracycle/blob/main/data/toronto-bikeways-preprocess-v4.csv)
...
# Save geojson
toron_preprocess %>%
	write_sf("../data/toronto-bikeways-preprocess-v4.geojson", delete_dsn = TRUE)
...
toron_preprocess %>%
	...
	write_csv("../data/toronto-bikeways-preprocess-v4.csv", na = "")
...
## Table 1: Comparison of Municipal Roadway and Bikeway Infrastructure in Vancouver, Calgary, and Toronto (Canada), 2022.

Information downloaded from municipally maintained open datasets Methodology and detailed download dates available in Appendix 2.

**Note**: Only measures from the bikeway data is calculated here.
...
# Calc lengths for city bikeway types
tab1$data <- vanc_preprocess %>%
	mutate(city = "vancouver") %>%
	select(city, install_type, road_type) %>%
	mutate(
		type = case_when(
			install_type == "Protected Bike Lanes" &
				road_type != "Off-street" ~ "Cycle Track (On-Street)",
			install_type == "Painted Lanes" &
				road_type != "Off-street" ~ "Painted Lane (On-Street)",
			install_type %in% c("Shared Lanes", "Local Street") &
				road_type != "Off-street" ~ "Shared Roads (On-Street)",
			.default = NA
		)
	) %>%
	add_row( # add lsbs for vanc
		vanc_preprocess %>%
			mutate(city = "vancouver") %>%
			select(city, install_type, road_type) %>%
			filter(install_type == "Local Street" & road_type != "Off-street") %>%
			mutate(type = "Local Street Bikeways (On-Street)")
	) %>%
	add_row(
		calg_preprocess %>%
			mutate(city = "calgary") %>%
			select(city, install_type, road_type) %>%
			mutate( # fix casing issues
				install_type = str_to_title(install_type)
			) %>%
			mutate(
				type = case_when(
					install_type == "Cycle Track" ~ "Cycle Track (On-Street)",
					install_type == "Bicycle Lane" ~ "Painted Lane (On-Street)",
					install_type %in% c(
						"Neighbourhood Greenway",
						"Shared Lane",
						"On-Street Bikeway"
					) ~ "Shared Roads (On-Street)",
					.default = NA
				)
			)
	) %>%
		...
		toron_preprocess %>%
			...
			mutate(
				type = case_when(
					...
					) ~ "Cycle Track (On-Street)",
					install_type %in% c(
						"Bike Lane",
						"Bike Lane - Buffered",
						"Bike Lane - Contraflow"
					) ~ "Painted Lane (On-Street)",
					str_starts(
						install_type,
...
tab1$data <- tab1$data %>%
	filter(city == "vancouver") %>%
	select(-city) %>%
	rename(Vancouver = len_km) %>%
	left_join(
		tab1$data %>% 
			filter(city == "calgary") %>%
			select(-city) %>%
			rename(Calgary = len_km),
		by = "type"
	) %>%
	left_join(
		tab1$data %>% 
			filter(city == "toronto") %>%
			select(-city) %>%
			rename(Toronto = len_km),
		by = "type"
	) %>%
	rename(Measure = type) %>%
	mutate(
		Title = "Municipal Bikeways and Pathways (2022, centreline-kilometers)",
		.before = 1
	)
	

# Create table for municipal data lengths
options(knitr.kable.NA = "")
tab1$table <- tab1$data %>%
	mutate(
		Vancouver = round(Vancouver, 1),
		Calgary = round(Calgary, 1),
		Toronto = round(Toronto, 1)
	) %>%
	kable(
		booktabs = T
	) %>%
...
* [manuscript/figures/tab-infra.pdf](https://github.com/rrwen/recovr-infracycle/blob/main/manuscript/figures/tab-infra.pdf)
* [manuscript/figures/tab-infra.png](https://github.com/rrwen/recovr-infracycle/blob/main/manuscript/figures/tab-infra.png)

```{r}

# Save pdf
if (!file.exists("../manuscript/figures/tab-infra.pdf")) {
	save_kable(tab1$table, "../manuscript/figures/tab-infra.pdf")
}

# Save png
if (!file.exists("../manuscript/figures/tab-infra.png")) {
	save_kable(tab1$table, "../manuscript/figures/tab-infra.png", zoom = 2)
}
...
# Save data
write_csv(tab1$data, "../data/plot/tab-infra.csv", na = "")
...
## Supplementary Table 2: Excluded Segment Types for Vancouver, Calgary, and Toronto (Canada). {.tabset}
...
* [manuscript/figures/tab-excl-infra.pdf](https://github.com/rrwen/recovr-infracycle/blob/main/manuscript/figures/tab-excl-infra.pdf)
* [manuscript/figures/tab-excl-infra.png](https://github.com/rrwen/recovr-infracycle/blob/main/manuscript/figures/tab-excl-infra.png)
...
Misc code added after feedback from reviewers.

## Misclassified Infrastructure Types

Calculate lengths of misclassified city infrastructure types.
...
misc1$data <- vanc %>%
	mutate(
		install_type2 = case_when(
			install_type == "Painted Lanes" ~ "PL",
			install_type == "Protected Bike Lanes" ~ "PBL",
			.default = NA
		),
		city = "vancouver"
	) %>%
	select(install_year, install_type2, verify_install_type, city) %>%
	add_row(
		calg %>%
		mutate(
			install_type2 = case_when(
				install_type == "Bicycle Lane" ~ "PL",
				install_type == "Cycle Track" ~ "PBL",
				.default = NA
			),
			city = "calgary"
		) %>%
	...
	add_row(
		...
		mutate(
			install_type2 = case_when(
				...
				install_type == "Bike Lane - Buffered" ~ "BUF",
				install_type %in% c(
					"Cycle Track",
					"Cycle Track - Contraflow"
				) ~ "PBL"
	...
	mutate(
		geometry_len_km = as.numeric(st_length(geometry) / 1000),
		install_type2 = if_else(
			install_type2 == "BUF",
			"PL",
			install_type2
		),
		verify_install_type = if_else(
			verify_install_type == "BUF",
			"PL",
			verify_install_type
		),
		misclass_type = if_else(
			install_type2 != verify_install_type,
			install_type2,
			NA
		),
		misclass_path = if_else(
			install_type2 != verify_install_type,
			paste0(install_type2, " -> ", verify_install_type),
			NA
		)
	)
...
misc1$data %>%
    as_tibble %>%
    select(-geometry) %>%
    group_by(city, misclass_path) %>%
    summarize(geometry_len_km = sum(geometry_len_km)) %>%
	filter(!is.na(misclass_path))
```

## Misclassified Types by Year

Plot misclassified city infrastructure types by year.
...
# Calc misclassified per year
misc2$data <- misc1$data %>%
	as_tibble %>%
	select(-geometry) %>%
	filter(
		!is.na(misclass_type) &
	...
	group_by(city, install_year, misclass_type) %>%
	summarize(geometry_len_km = sum(geometry_len_km)) %>%
	left_join(
		misc1$data %>%
			as_tibble %>%
			filter(!is.na(install_type2)) %>%
			group_by(city, install_year, install_type2) %>%
			summarize(total_len = sum(geometry_len_km)) %>%
			mutate(misclass_type = install_type2) %>%
			select(-install_type2),
		by = c("city", "install_year", "misclass_type")
	) %>%
	mutate(
		year = install_year,
		percentage = (geometry_len_km / total_len) * 100,
		infrastructure_type = case_when(
			misclass_type == "PBL" ~ "Cycle Track",
			misclass_type == "PL" ~ "Painted and Buffered Lane",
			.default = NA
		),
		city = str_to_title(city)
	)

# Create bar plot of misclass per year
misc2$plot <- ggplot(
	misc2$data,
	aes(
		x = install_year,
		y = round(percentage, 1),
		fill = infrastructure_type
	)
) +
  geom_bar(stat = "identity", position = "stack") +
  geom_label(
    aes(label = paste0(round(geometry_len_km, 1), " km")),
...
```{r}
# Save data
write_csv(misc2$data, "../data/plot/fig-yearly-misclass.csv", na = "")

# Display data
misc2$data %>%
	datatable(filename = "fig-yearly-misclass")
```
...
# Save pdf
ggsave(
	"../manuscript/figures/fig-yearly-misclass.pdf",
	misc2$plot,
	width = 8.5,
	height = 11
)
...
# Save png
ggsave(
	"../manuscript/figures/fig-yearly-misclass.png",
	misc2$plot,
	width = 8.5,
	height = 11
)
