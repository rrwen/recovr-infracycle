...
tab1$data_raw_cols <- c("city", "install_type", "install_type2", "verify_final_type", "road_type")
tab1$data_raw <- vanc %>%
	mutate(city = "vancouver") %>%
	select(tab1$data_raw_cols) %>%
	add_row(
		calg %>%
			mutate(city = "calgary") %>%
			select(tab1$data_raw_cols)
...
tab1$data <- tab1$data  %>%
	...
	mutate(
		len_km_diff_label = glue(
			"{if_else(len_km_diff > 0, '+', '')}{len_km_diff_rd} ({len_km_perc_rd}%)"
		),
		len_km_diff_ci_label = glue(
			"{if_else(len_km_diff_ci_lower > 0, '+', '')}{len_km_diff_ci_lower_rd} ({if_else(len_km_diff_ci_lower > 0, '+', '')}{len_km_diff_ci_lower_perc_rd}%), ",
			"{if_else(len_km_diff_ci_upper > 0, '+', '')}{len_km_diff_ci_upper_rd} ({if_else(len_km_diff_ci_upper > 0, '+', '')}{len_km_diff_ci_upper_perc_rd}%)"
		)
	) %>%
	arrange(city) %>%
	select(
...
# Create data
misc1$data_cols <- c("city", "install_type", "install_type2", "verify_final_type", "road_type")
misc1$data <- vanc %>%
	mutate(city = "vancouver") %>%
	select(misc1$data_cols) %>%
	add_row(
		calg %>%
		mutate(city = "calgary") %>%
		select(misc1$data_cols)
