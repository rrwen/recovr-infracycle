...
#' @param line_50km Set to `TRUE` to add a 50km reference line.
...
diag_criteria <- function(
		criteria_data,
		note = "*Denotes segments misclassified as an ineligible type (off-street path, shared road, or inactive temporary infrastructure)<br/>**Local Street Bikeways (LSB) were included but not screened or verified"
) {
...
plot_yearly_change <- function(
	...
	for (i in 1:length(df_list)) {
		...
		# Calc infra per year
		pdata[[i]] <- calc_yearly_adj_len(
			df,
			type_col = settings$type_col_infra
		)
		
		# Calc total infra per year
		pdata[[i]] <- pdata[[i]] %>% add_row(
			pdata[[i]] %>% group_by(
				year
			) %>% summarize(
				adj_len = sum(adj_len)
			) %>% mutate(
				verify_install_type = "TOTAL"
			)
		) %>% mutate(
			city = city
		)
	...
	p[[3]] <- pdata %>% filter(
		verify_install_type %in% c("PL", "BUF")
	) %>% group_by(year, city) %>% summarize(
		adj_len_norm = sum(adj_len_norm, na.rm = TRUE)
	) %>% group_by(city) %>% arrange(year) %>% mutate(
		change = adj_len_norm - lag(adj_len_norm)
		...
		bquote(underline(.("Painted Lanes")))
	) + scale_y_continuous(
		label = scales::label_number(suffix = " km")
	) + scale_colour_manual(
		values = c("#546ca9", "#c5a43d", "#719d71")
...
* [manuscript/figures/table-yearly-len.pdf](https://github.com/rrwen/recovr-infracycle/blob/main/manuscript/figures/table-yearly-len.pdf)
* [manuscript/figures/table-yearly-len.png](https://github.com/rrwen/recovr-infracycle/blob/main/manuscript/figures/table-yearly-len.png)
