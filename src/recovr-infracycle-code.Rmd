...
diag_criteria <- function(
	...
	for (i in 1:length(criteria_list)) {
		...
		elig_inelig <- glue(
			"<br/>Ineligible: n = {n}",
			n = criteria[["infra_filter_nx"]] + criteria[["status_filter_nx"]]
		)
		...
		row_adds <- paste0(row_adds, glue(
			...
			open_len = round(criteria[["data_len"]], 1),
			open_date = criteria[["data_date"]],
			elig_n = criteria[["elig_n"]],
			elig_len = round(criteria[["elig_len"]], 1),
			elig_inelig = elig_inelig,
			elig_dup = elig_dup,
			elig_poly = elig_poly,
			incl_n = criteria[["incl_n"]],
			incl_len = round(criteria[["incl_len"]], 1),
			incl_miss = incl_miss,
			incl_dup = ""
		))
