...
plot_yearly_len <- function(
	...
	df <- df %>% filter(
		!.data[[cat_col]] %in% cat_filter
	)
	
	# Recode category types
	df <- df %>% mutate(
		!!cat_col := recode(.data[[cat_col]], !!!cat_recode)
	)
	
	# Create fill colors
	cat_n <- length(unique(df[[cat_col]]))
	cat_colors <- scales::seq_gradient_pal(
		color_low,
		color_high
	)(seq(0, 1, length.out = cat_n))
