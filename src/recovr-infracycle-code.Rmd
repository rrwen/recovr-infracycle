...
#'	\item \code{status_col}: the column name (char) of the column containing the active road status types to filter.
#'	\item \code{status_filter}: the vector of characters of active road status types to include.
...
#'	\item \code{status_filter_applied}: TRUE if active road status filter was applied and FALSE otherwise (required).
#'	\item \code{status_filter_n}: total rows (numeric) after filtering for active road status using \code{status_filter} (required).
#'	\item \code{status_filter_len}: total road length (numeric) after filtering for active road status using \code{status_filter} (required).
#'	\item \code{status_filter_nx}: total rows (numeric) affected by filtering for active road status using \code{status_filter} (required).
#'	\item \code{status_filter_lenx}: total road length (numeric) affected by filtering for active road status using \code{status_filter} (required).
...
filter_criteria <- function(criteria_list) {
	...
	for (i in 1:length(criteria_list)) {
		...
		out[[i]]$status_filter_nx <- out[[i]]$infra_filter_n - out[[i]]$status_filter_n
		out[[i]]$status_filter_lenx <- out[[i]]$infra_filter_len - out[[i]]$status_filter_len
...
## Function 5a: draw_criteria_diagram

Draws a method diagram.

This function draws a flow diagram of overall methods for the paper using the output from `filter_criteria`.

```{r}
draw_methods_diagram <- function(criteria_list) {
	...
	for (criteria in criteria_list) {
		...
		# Calculate inelig
		elig_inelig <- criteria$data_n - criteria$elig_n
		elig_poly <- elig_inelig - criteria$geom_filter_null_n
		elig_dup <- elig_poly - criteria$geom_filter_dup_n
		incl_dup <- ""
		...
		row_adds <- paste0(row_adds, glue(
			...
			open_len = criteria[["data_len"]],
			open_date = criteria[["data_date"]],
			elig_n = criteria[["elig_n"]],
			elig_len = criteria[["elig_len"]],
			elig_inelig = elig_inelig,
			elig_dup = elig_dup,
			elig_poly = elig_poly,
			incl_n = criteria[["incl_n"]],
			incl_len = criteria[["incl_len"]],
			incl_miss = criteria[["misclass_filter_n"]],
			incl_dup = incl_dup
		))
