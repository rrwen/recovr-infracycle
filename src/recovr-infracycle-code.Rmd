...
#' 
#' @return A ggplot of yearly differences (year_col2 - year_col1), displaying the proportion of rows for each difference in years. If \code{out_data} is TRUE then returns a list with keys `data` representing the data used for plotting and \code{plot} with the ggplot object.
#' @export
#'
...
#' This function applies segment inclusion critieria to a list of data.frames. Optionally creates a data.frame of counts, segment lengths, and other exclusions (duplicates, misclassifications) per inclusion criteria step along with a list of the data.frames after applying the inclusion criteria.
#'
#' @param criteria_list A list of lists, where each list contains the following structure defining the segment inclusion criteria for each city:
#' \itemize{
...
#' }
#' 
#' @return A list of lists, where each list has keys and values from \code{criteria_list}, and the following additional keys:
#' \itemize{
...
#' @export
#'
filter_criteria <- function(criteria_list) {
	...
	for (i in 1:length(criteria_list)) {
		...
		out[[i]]$infra_filter_applied <- FALSE
		out[[i]]$road_filter_applied <- FALSE
		out[[i]]$status_filter_applied <- FALSE
		out[[i]]$geom_filter_null_applied <- FALSE
		out[[i]]$geom_filter_dup_applied <- FALSE
		out[[i]]$misclass_filter_applied <- FALSE
		
		# Count/len initial
		out[[i]]$data_n <- nrow(df)
		out[[i]]$data_len <- sum(st_length(df[[x$geom_col]]), na.rm = TRUE)
		...
		if (all(c("infra_col", "infra_filter") %in% names(x))) {
			...
			out[[i]]$infra_filter_applied <- TRUE
		}
		
		# Count/len ded cyc infra filter
		out[[i]]$infra_filter_n <- nrow(df)
		out[[i]]$infra_filter_len <- sum(st_length(df[[x$geom_col]]), na.rm = TRUE)
		
		# Count/len affected by ded cyc infra filter
		out[[i]]$infra_filter_nx <- out[[i]]$data_n - out[[i]]$infra_filter_n
		out[[i]]$infra_filter_lenx <- out[[i]]$data_len - out[[i]]$infra_filter_len
		
		# Filter for road location
		if (all("road_col", "road_filter") %in% names(x)) {
			
			# Apply road filter
			df <- df %>%
				filter(!.data[[x$status_col]] %in% x$road_filter)
			
			# Set road filter status
			out[[i]]$road_filter_applied <- TRUE
		}
		
		# Count/len road filter
		out[[i]]$road_filter_n <- nrow(df)
		out[[i]]$road_filter_len <- sum(st_length(df[[x$geom_col]]), na.rm = TRUE)
		
		# Count/len affected by road filter
		out[[i]]$road_filter_nx <- out[[i]]$infra_filter_n - out[[i]]$road_filter_n
		out[[i]]$road_filter_lenx <- out[[i]]$infra_filter_len - out[[i]]$road_filter_len
		...
		if (all(c("status_col", "status_filter") %in% names(x))) {
			...
			out[[i]]$status_filter_applied <- TRUE
		}
		
		# Count/len status filter
		out[[i]]$status_filter_n <- nrow(df)
		out[[i]]$status_filter_len <- sum(st_length(df[[x$geom_col]]), na.rm = TRUE)
		
		# Count/len affected by status filter
		out[[i]]$status_filter_nx <- out[[i]]$road_filter_n - out[[i]]$status_filter_n
		out[[i]]$status_filter_lenx <- out[[i]]$road_filter_len - out[[i]]$status_filter_len
		...
		if (all(c("geom_col", "geom_filter") %in% names(x))) {
			...
			out[[i]]$geom_filter_null_applied <- TRUE
		}
		
		# Count/len null geom filter
		out[[i]]$geom_filter_null_n <- nrow(df)
		out[[i]]$geom_filter_null_len <- sum(st_length(df[[x$geom_col]]), na.rm = TRUE)
		
		# Count/len affected by null geom filter
		out[[i]]$geom_filter_null_nx <- out[[i]]$status_filter_n - out[[i]]$geom_filter_null_n
		out[[i]]$geom_filter_null_lenx <- out[[i]]$status_filter_len - out[[i]]$geom_filter_null_len
		...
		if (all(c("geom_col", "geom_filter") %in% names(x))) {
			...
			# Set dup geom filter status
			out[[i]]$geom_filter_dup_applied <- TRUE
		}
		
		# Count/len dupl geom filter
		out[[i]]$geom_filter_dup_n <- nrow(df)
		out[[i]]$geom_filter_dup_len <- sum(st_length(df[[x$geom_col]]), na.rm = TRUE)
		
		# Count/len affected by dupl geom filter
		out[[i]]$geom_filter_dup_nx <- out[[i]]$geom_filter_null_n - out[[i]]$geom_filter_dup_n
		out[[i]]$geom_filter_dup_lenx <- out[[i]]$geom_filter_null_len - out[[i]]$geom_filter_dup_len
		
		# Count/len eligible
		out[[i]]$elig_n <- nrow(df)
		out[[i]]$elig_len <- sum(st_length(df[[x$geom_col]]), na.rm = TRUE)
		
		# Filter for misclass
		if (all(c("misclass_col", "misclass_filter") %in% names(x))) {
			
			# Count/len misclass groups
			out[[i]]$misclass_filter_uniq_n <- df %>%
				filter(!is.na(.data[[x$misclass_col]])) %>%
				count(.data[[x$misclass_col]]) %>%
				as_tibble
			out[[i]]$misclass_filter_uniq_len <- df %>%
				filter(!.data[[x$misclass_col]] %in% x$misclass_filter) %>%
				group_by(.data[[x$misclass_col]]) %>%
				summarize(len = sum(st_length(.data[[x$geom_col]]), na.rm = TRUE)) %>%
				as_tibble
			...
			out[[i]]$misclass_filter_applied <- TRUE
		}
		
		# Count/len misclass filter
		out[[i]]$misclass_filter_n <- nrow(df)
		out[[i]]$misclass_filter_len <- sum(st_length(df[[x$geom_col]]), na.rm = TRUE)
		
		# Count/len affected by misclass filter
		out[[i]]$misclass_filter_nx <- out[[i]]$geom_filter_dup_n - out[[i]]$misclass_filter_n
		out[[i]]$misclass_filter_lenx <- out[[i]]$geom_filter_dup_len - out[[i]]$misclass_filter_len
		
		# Count/len eligible
		out[[i]]$incl_n <- nrow(df)
		out[[i]]$incl_len <- sum(st_length(df[[x$geom_col]]), na.rm = TRUE)
		
		# Save filtered data
		out[[i]]$data_filter <- df
	}
	return(out)
}
```
...
#'
#' @param criteria_list A list of lists in the structure of the output from \code{\link{filter_criteria}}.
...
diag_criteria <- function(
		criteria_list,
		note = "*Denotes segments misclassified as an ineligible type (off-street path, shared road, or inactive temporary infrastructure)"
) {
	...
	row_temp <- "
		...
		elig{i}[
			label = <n = {elig_n} Segments<br/>({elig_len})<br/><i><b>Exclusions</b>{elig_inelig}{elig_dup}{elig_poly}</i>>,
			pos = '3.25,{y}!'
		]
	...
	for (i in 1:length(criteria_list)) {
		...
		row_adds <- paste0(row_adds, glue(
			...
			elig_n = criteria[["elig_n"]],
			elig_len = paste(round(criteria[["elig_len"]], 1), len_unit),
			elig_inelig = elig_inelig,
			elig_dup = elig_dup,
			elig_poly = elig_poly,
			incl_n = criteria[["incl_n"]],
			incl_len = paste(round(criteria[["incl_len"]], 1), len_unit),
			incl_miss = incl_miss,
			incl_dup = ""
		))
...
#' @return A list of \code{\link[DiagrammeR]{grViz}} objects if `city` is `NULL`, or a single \code{\link[DiagrammeR]{grViz}} if `city` is provided.
#' @export
#'
diag_criteria_details <- function(criteria_list, city = NULL) {
	...
	for (i in 1:length(criteria_list)) {
		...
		infra_keys <- c("infra_col", "infra_filter", "infra_filter_n", "infra_filter_nx")
		if (all(infra_keys %in% names(criteria))) {
			fi <- fi + 1
			y <- y - 2.25
			s2 <- glue("
				{s2}
		...
		road_keys <- c("road_col", "road_filter", "road_filter_n", "road_filter_nx")
		if (all(road_keys %in% names(criteria))) {
			fi <- fi + 1
			y <- y - 2.25
			s2 <- glue("
				{s2}
				...
				filter{fi}[
					label = 'Filter for Infrastructure Located on Roadway\\l{column} in {filter}\\l(n = {n})\\l'
					pos = '-2,{y}!'
				]
				filter{fi}x[
					label = 'Segments Excluded\\l(n = {nx})\\l'
		...
		status_keys <- c("status_col", "status_filter", "status_filter_n", "status_filter_nx")
		if (all(status_keys %in% names(criteria))) {
			fi <- fi + 1
			y <- y - 2.25
			s2 <- glue("
				{s2}
		...
		# Step 2 filtering geom
		geom_keys <- c("geom_col", "geom_filter", "geom_filter_n", "geom_filter_nx")
		if (all(geom_keys %in% names(criteria))) {
			...
			",
				...
				n = criteria$geom_filter_n,
				nx = criteria$geom_filter_nx,
				fi = fi,
				y = y,
				x = x_edge,
				s2 = s2
		...
		s4 <- glue("
			...
			screen[
				label = 'Exclusion of Misclassifications and\\lDuplicates following Screening\\l{column} != {filter}\\l(n = {n})\\l'
				pos = '-4.5,{y}!'
				width = 5
				height = 2.5
			]
		...
		s5 <- glue("
			...
			incl[
				label = 'Verified Inclusions\\l(n = {n})\\l'
				pos = '0,{y}!'
				width = 14
			]
		",
...
```{r}
vanc_raw <- read_sf("../data/raw/vancouver/Vancouver AS 3666 V2.shp")
```
...
vanc <- vanc_raw %>%
	...
	mutate( # data types
		...
		verify_exclude = as.character(verify_exclude)
	) %>%
	mutate( # clean values
		...
		verify_exclude =  na_if(verify_exclude, "NA") %>%
			str_trim %>%
			str_to_title
	) %>%
	mutate( # add local street as LSB to verify installs
...
# Save geojson
vanc %>%
	st_write("../data/vancouver_verify_bikeways_v1.geojson", delete_dsn = TRUE)

# Save csv
# st_read("../data/vancouver_bikeways_v1.csv", options = "GEOM_POSSIBLE_NAMES=geometry", crs = "urn:ogc:def:crs:OGC:1.3:CRS84")
vanc %>%
	mutate(
		geometry = st_as_text(geometry),
		geometry_crs = st_crs(vanc)$proj4string,
		.before = geometry
	) %>%
	write_csv("../data/vancouver_verify_bikeways_v1.csv", na = "")
```
...
# Save geojson
toron %>%
	st_write("../data/toronto_verify_bikeways_v1.geojson", delete_dsn = TRUE)
...
toron %>%
	mutate(
		geometry = st_as_text(geometry),
		geometry_crs = st_crs(toron)$proj4string,
		.before = geometry
	) %>%
	write_csv("../data/toronto_verify_bikeways_v1.csv", na = "")
```
...
criteria_settings <- list(
	list(
		...
		infra_filter = c("Painted Lanes", "Protected Bike Lanes"),
		geom_col = "geometry",
		geom_unit = "km",
		geom_filter = TRUE,
...
criteria_list <- filter_criteria(criteria_settings)
diag_criteria(criteria_list)
```
...
plot_yearly_len_infra(list(
	"Vancouver, CA" = vanc %>% as_tibble,
	#"Calgary, CA" = calg %>% as_tibble,
	"Toronto, CA" = toron %>% as_tibble
))
```
...
```
id -> infra_filter [minlen = 3]
subgraph cluster1 {
	
		label = <<br/><b>    2. Filtering</b>>
		margin = 30
		bgcolor = '#d7e9fe'
		
		infra_filter -> road_filter -> geom_filter
		{rank=same infra_filter -> infra_filterx [minlen = 2]}
		{rank=same road_filter -> road_filterx [minlen = 2]}
		{rank=same geom_filter -> geom_filterx [minlen = 2]}
	}
	geom_filter -> elig [minlen = 3]
	
	subgraph cluster2 {
	
		label = <<br/><b>    3. Eligible</b>>
		margin = 30
		bgcolor = '#d7e9fe'
		...
		{
			rank = same
			elig -> eligv [style = invis, minlen = 2]
		}
	}
	elig -> screen [minlen = 3]
	
	subgraph cluster3 {
	
		label = <<br/><b>    4. Screening</b>>
		margin = 30
		bgcolor = '#d7e9fe'
		
		{rank=same screen -> screenx [minlen = 2]}
	}
	screen -> inclu [minlen = 3]
	
	subgraph cluster4 {
	
		label = <<br/><b>    5. Inclusions</b>>
		bgcolor = '#c8e29d'
		margin = 30
		
		incluv[style=invis]
		{rank=same inclu -> incluv [style = invis, minlen = 2]}
	}
```
...
digraph{
	...
	id[
		label = 'Shapefile from: https://data.calgary.ca/Transportation-Transit/Calgary-Bikeways/jjqk-9b73\\lDownloaded: January 2023\\lN = 1000 Segments\\l'
		pos = '0,0!'
		width = 14
	]
	...
	filter1[
		label = 'Filter for Dedicated Cycling Infrastructure\\lbicycle class = “Bicycle - Lane” or “Cycle Track”.\\l(n = 795)\\l'
		pos = '-2,-2!'
	]
	filter1x[
		label = 'Segments Excluded\\l(n = 992)\\l'
		pos = '5.5,-2!'
		width = 3
	]
	filter1 -> filter1x
	...
	filter2[
		label = 'Remove null geometries and check for duplicated segments\\l(n = 782)\\l'
		pos = '-2,-3.85!'
	]
	filter2x[
		label = 'Segments Excluded\\l(n = 992)\\l'
