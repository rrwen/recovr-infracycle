...
plot_yearly_len <- function(
	...
	scale_x_continuous(
		breaks = seq(year_start, year_end, by = year_int),
		labels = seq(year_start, year_end, by = year_int),
...
# All plots together
(y_title | wrap_plots(list(vanc_plot_infra, calg_plot_infra, toron_plot_infra), nrow = 3)) +
	plot_annotation(
		title = "Roadways with Dedicated Cycling Infrastructure",
		caption = "Years (2009-2022)",
		theme = theme(
...
vanc_plot_roads <- list()

# Plot overall road types
vanc_plot_roads[[1]] <- calc_yearly_len(
	vanc,
	year_col = "install_year",
	type_col = "road_type"
) %>%
...
for (i in 1:length(rtypes)) {
	...
	vanc_plot_roads[[i + 1]] <- calc_yearly_adj_len(
		vanc %>% filter(road_type == r),
		type_col = "infra_type"
	) %>%
