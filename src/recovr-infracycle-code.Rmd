...
filter_criteria <- function(
	...
	for (i in 1:length(criteria_list)) {
		...
		out[[city]]$misclass_filter_n <- nrow(df)
		out[[city]]$misclass_filter_len <- len_func(sum(st_length(df[[x$geom_col]]), na.rm = TRUE))
		
		# Count/len affected by misclass filter
		out[[city]]$misclass_filter_nx <- out[[city]]$geom_filter_dup_n - out[[city]]$misclass_filter_n
		out[[city]]$misclass_filter_lenx <- out[[city]]$geom_filter_dup_len - out[[city]]$misclass_filter_len
		
		# Count/len eligible
		out[[city]]$incl_n <- nrow(df)
		out[[city]]$incl_len <- len_func(sum(st_length(df[[x$geom_col]]), na.rm = TRUE))
...
#' @export
#'
diag_criteria_details <- function(criteria_list, city = NULL, out_render = TRUE) {
	
	# Filter for city if avail
	if (!is.null(city)) {
		criteria_list <- criteria_list[sapply(criteria_list, function (x) x$city == city)]
	}
...
```{r}
diag_criteria_details(criteria_list, "vancouver")
```
...
```{r}
diag_criteria_details(criteria_list, "toronto")
```
