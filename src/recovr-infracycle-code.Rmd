...
4. In RStudio, open the [src/recovr-infracycle-code.Rmd](https://github.com/rrwen/recovr-infracycle/blob/main/src/recovr-infracycle-code.Rmd) file
6. In RStudio:
...
tryCatch(
	{
		...
		for (package in required_packages) {
			...
			if (!is_package_installed & package != "osmplotr") { # CRAN packages
				cat(paste0("Installing package: ", package, "\n"))
				install.packages(package)
			} else if (!is_package_installed & package == "osmplotr") { # Github packages
				cat(paste0("Installing package (development version): ", package, "\n"))
...
## Function 6: map_infra

Maps Infrastructure Changes.

Creates maps of infrastructure changes since a certain year for each city and their downtown region.
...
map_infra <- function(
	...
	for (i in 1:length(cities_list)) {
		...
		# Get vars
		city <- cities_list[[i]]
		city_name <- names(cities_list)[[i]]
		dt_name <- glue(
			"Downtown {city}",
			city = str_to_title(city_name)
		)
		city$zoom <- if ("zoom" %in% names(city)) city$zoom else 12
		city$basemap_extend <- if ("basemap_extend" %in% names(city)) city$basemap_extend else 10000
		...
		# Create col for city
		map_data <- map_data %>%
			mutate(group = city$title)
		
		# Create bounding box for downtown region
		bbox <- st_as_sfc(
			st_bbox(city$downtown_bbox),
			crs = 4326
		)
		
		# Subset data for downtown region
		submap_data <- map_data %>%
			st_crop(bbox) %>%
			mutate(group = dt_name)
		
		# Add bounding box as polylines for main map
		map_data <- map_data %>%
			add_row(
				st_sf(
					geometry = bbox,
		...
		# Combine map data into single sdf
		all_data <- bind_rows(list(map_data, submap_data)) %>%
			mutate(group = factor(
				group,
				levels = c(city$title, dt_name)
			))
		
		# Create color palette
		colors <- c("green", "orange", "gray", "red")
		names(colors) <- c(
		...
		# Create the base map with extended boundaries
		ext_bounds <- st_buffer(
			st_as_sfc(st_bbox(map_data)),
			dist = city$basemap_extend
		)
		basemap <- read_osm(
			ext_bounds,
			type = settings$static_basemap,
			zoom = city$zoom
		)
		
		# Set map plotting mode
		current_mode <- tmap_mode()
		tmap_mode("plot")
		
		# Generate map
		out[[i]] <- tm_shape(basemap) +
			tm_rgb() +
			tm_shape(all_data) +
			tm_lines(
				col = "changes",
			...
			tm_compass(position = "right", type = "arrow") +
			tm_facets(by = "group") +
			tm_layout(
				legend.outside = TRUE,
		...
		if (i == length(cities_list)) {
			out[[i]] <- out[[i]] +
				tm_add_legend(
					title = "Legend",
					type = "line",
					lwd = 4,
					col = c("green", "orange", "gray"),
					labels = c(
						glue(
							"New Infrastructure Since Jan. {year}",
							year = year_since
	...
	# Combine maps
	out <- tmap_arrange(out, ncol = 1)
	
	# Reset map plotting mode and return map
...
New installations of dedicated infrastructure are denoted in green, upgrades from a previous dedicated infrastructure type are denoted in orange.


```{r map-infra, warning = FALSE, message = FALSE, eval = FALSE}
map_infra(list(
	toronto = list(
		title = "Toronto, CA",
		data = toron,
...
)) #%>% tmap_save("../manuscript/fig-map-infra.pdf", dpi = 600)
```

```{r}
bbox <- get_bbox(st_bbox(toron))
water <- extract_osm_objects(
	key = "natural",
	value = "water",
	bbox = bbox)
river <- extract_osm_objects(
	key = "waterway",
	value = "river",
	return_type = "line",
	bbox = bbox
)
lake <- extract_osm_objects(
	key = "name",
	value = "Lake Ontario",
	return_type = "multipolygon",
	bbox = bbox
)
highway_main <- extract_osm_objects(
	key = "highway",
	value = "motorway|trunk",
	bbox = bbox
)
highway <- extract_osm_objects(
	key = "highway",
	value = "primary|unclassified",
	bbox = bbox
)
```
```{r}
ggplot() +
	geom_sf(data = lake %>% st_crop(st_bbox(toron)), fill = "gray90", color = "gray90") +
	geom_sf(data = water %>% st_crop(st_bbox(toron)), size = 0.75, color = "gray90") +
	geom_sf(data = river %>% st_crop(st_bbox(toron)), size = 0.5, color = "gray90") +
	geom_sf(data = highway_main %>% st_crop(st_bbox(toron)), color = "white") +
	geom_sf(data = highway %>% st_crop(st_bbox(toron)), size = 0.5, color = "white") +
	geom_sf(data = toron, aes(color = verify_install_type)) +
	annotation_scale(
		location = "bl"
	) +
...
Any data where a city provided and verified installation years were missing or the verified year occurred earlier than the start of the study period (2009) has been excluded from analysis, yielding n=` sfig7_n` segments. The graph shows that ` sfig7_0`% of the included segments had the correct installation year as per the city's data, and ` sfig7_pm1`% were accurate within a range of ±1 year.
