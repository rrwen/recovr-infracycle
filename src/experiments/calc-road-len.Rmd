...
#' @param is_len_meters Set to TRUE or FALSE indicating if the road lengths are in meters.
#'
...
## Step 2a: Calculate Yearly Road Lengths By Infrastructure Type for Upgrade 1
...
## Step 2b: Calculate Yearly Road Lengths for Replaced Infrastructure Types from Upgrade 1
...
## Step 3a: Calculate Yearly Road Lengths By Infrastructure Type for Upgrade 2
...
## Step 3b: Calculate Yearly Road Lengths for Replaced Infrastructure Types from Upgrade 2
...
```{r}
vanc_yearly_len %>% filter(year >= 2009) %>% mutate(
	type = factor(type, levels = rev(c("PL", "LSB", "BUF", "PBL")))
) %>% ggplot(aes(
	x = year,
	y = len,
	fill = type
